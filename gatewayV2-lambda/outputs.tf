...
output "apigatewayv2_api_execution_arn" {
  ...
  value       = taws_apigatewayv2_api.attielabz-gateway.execution_arn
}
...
output "default_apigatewayv2_stage_arn" {
  ...
  value       = aws_apigatewayv2_stage.dattielabz-gateway-stage.arn
}

output "default_apigatewayv2_stage_execution_arn" {
  description = "The ARN prefix to be used in an aws_lambda_permission's source_arn attribute or in an aws_iam_policy to authorize access to the @connections API."
  value       = aws_apigatewayv2_stage.dattielabz-gateway-stage.execution_arn
}
